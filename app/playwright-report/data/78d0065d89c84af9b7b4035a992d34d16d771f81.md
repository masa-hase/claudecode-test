...
- Name: MySQL Analyzer >> should display tuning suggestions for problematic queries
- Location: /workspaces/claudecode-test/app/e2e/mysql-analyzer.spec.ts:56:7
...
>  56 |   test('should display tuning suggestions for problematic queries', async ({ page }) => {
  ...
  112 |
  113 |     // Wait for results
  114 |     await expect(page.getByText('解析結果')).toBeVisible();
  115 |     await expect(page.getByText('users')).toBeVisible();
  116 |     await expect(page.getByText('range')).toBeVisible();
  117 |     await expect(page.getByText('idx_age')).toBeVisible();
  118 |   });
  119 | });
```